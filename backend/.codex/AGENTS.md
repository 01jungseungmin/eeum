...
This file provides durable repository guidance to Codex when working with this backend.

Codex reusable workflows live under `.codex/skills/`. When a request matches a skill description, read that skill's `SKILL.md` completely before acting. Shared detailed rules live under `.codex/references/`.
...
- 결제, 환불, 취소, 수익 원장, 정산, PortOne 변경 및 전체 리뷰는
  `.codex/references/payment-settlement.md`를 반드시 읽고 적용한다.
