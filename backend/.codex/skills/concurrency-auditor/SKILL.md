...
description: "재고, 주문, 결제 Webhook, 스케줄러, 환불, 예약, 채팅방, 카운터 코드의 동시성, 멱등성, race condition, 중복 처리를 감사할 때 사용한다."
---
...
`.codex/references/concurrency/`에 도메인별 알려진 경쟁 시나리오가 정리되어 있다.
...
결제·환불·취소·수익 원장·정산·PortOne이 포함되면
`.codex/references/payment-settlement.md`도 처음부터 끝까지 읽고,
내부 상태와 외부 상태 사이의 경쟁까지 감사한다.
