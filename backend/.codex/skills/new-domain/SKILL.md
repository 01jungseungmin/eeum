---
name: new-domain
description: "사용자가 이음 백엔드에 신규 도메인을 프로젝트 표준 4-layer 구조로 생성하거나 스캐폴딩해 달라고 요청할 때 사용한다."
---

사용자가 지정한 도메인을 이음 프로젝트 표준 구조로 생성한다.

## 작업 순서

1. 도메인명을 패키지명, 클래스명, URL 리소스명(복수형 kebab-case)으로 변환한다.
2. 기존 도메인(store 또는 order)의 실제 코드를 읽고 패키지/네이밍/작성 패턴을 확인한다.
3. `.codex/references/layer-rules.md`의 레이어별 규칙에 따라
   4-layer 구조(api/{domain} · application/{domain}의 service/dto/mapper · domain/{domain}의 entity/repository/enums)에 파일을 생성한다.
4. URL, 인증, 예외, 삭제, 알림 정책은 `.codex/references/policies.md`를 따른다.
5. 완료 후 `.codex/references/output.md` 형식으로 요약한다.
