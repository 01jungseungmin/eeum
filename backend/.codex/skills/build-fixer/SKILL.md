---
name: build-fixer
description: "Gradle 빌드나 테스트가 실패했거나 QueryDSL Q 클래스, 컴파일 오류, 깨진 테스트를 진단하고 수정해야 할 때 사용한다."
---
...
4. 최대 3회 시도 후에도 실패하면 시도 내역과 함께 사용자에게 보고한다
...
- 어느 쪽이 맞는지 모호하면 수정하지 말고 사용자에게 판단을 요청한다
- 이음 코딩 표준(.codex/AGENTS.md)을 위반하는 방식으로 수정하지 않는다
