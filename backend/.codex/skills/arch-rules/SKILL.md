---
name: arch-rules
description: "AGENTS.md 절대 규칙과 자원 점유 금지 패턴을 ArchUnit 테스트로 고정하도록 요청했을 때 사용한다. 아키텍처 규칙 검증, 레이어 의존 방향, 엔티티 작성 규칙, 금지 패턴을 CI에서 정적으로 막고 싶을 때 사용한다."
---

사용자가 지정한 범위의 아키텍처 규칙을 ArchUnit 테스트로 작성하거나 갱신한다.
범위 지정이 없으면 전체 규칙 세트를 점검하고 누락된 규칙만 추가한다.

## 목적

`.codex/AGENTS.md`의 "절대 규칙"과 `.codex/references/resource-budget.md`의 금지 패턴은 지금 **사람이 리뷰로만** 지키고 있다.
...
1. `.codex/references/resource-budget.md`를 읽는다 — 금지 패턴의 단일 기준이다.
2. `.codex/AGENTS.md`의 "절대 규칙", "엔티티 작성 규칙", "트랜잭션 분리" 절을 읽는다.
...
4. `build.gradle`의 `test` 태스크에 실패 로그 설정이 있는지 확인한다. 없으면 추가한다:
   ...
   ```
   **이 설정이 없으면 이 skill은 무의미하다.** ArchUnit은 위반 목록을 예외 메시지 본문에만 담기 때문에,
...
### ResourceRuleTest — 실제 장애에서 도출된 규칙 (최우선)
