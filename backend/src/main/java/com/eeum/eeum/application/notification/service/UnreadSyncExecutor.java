...
public class UnreadSyncExecutor {
    ...
    public void rebuildAndPush(Long accountId) {
        rebuildAndPushOne(accountId);
    }

    // 참여자 전원을 한 작업으로 처리한다 — 인원수만큼 async 작업을 제출하면
    // 채팅방 하나 종료에 큐가 통째로 차고, 그 포화가 알림 생성 작업까지 밀어낸다.
    @Async("asyncTaskExecutor")
    public void rebuildAndPushAll(List<Long> accountIds) {
        if (accountIds == null) return;
        accountIds.forEach(this::rebuildAndPushOne);
    }

    // refreshFromDb가 방금 만든 스냅샷을 그대로 싣는다.
    // 여기서 getUnreadCount를 다시 부르면 트랜잭션과 Redis 왕복이 한 번 더 생긴다.
    private void rebuildAndPushOne(Long accountId) {
        try {
            realtimeRelayPublisher.publishUnreadCount(
                    accountId, unreadCountService.refreshFromDb(accountId));
