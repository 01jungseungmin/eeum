 ...
 * outbox 행 하나를 처리한다.
 *
 * 알림 생성과 상태 전이를 한 트랜잭션에, 그리고 그 트랜잭션을 행 잠금 뒤에
 * 두는 것이 이 클래스의 존재 이유다.
...
public class NotificationOutboxDispatcher {
    ...
    /**
     * 행을 잠그고 읽는다. 상태 확인과 처리 사이에 다른 인스턴스가 같은 행을 집으면
     * 같은 알림이 두 번 만들어진다. 폴링을 한 인스턴스로 좁히는 @SchedulerLock의 lease는
    ...
    /**
     * 실패 기록은 별도 트랜잭션이다. 처리 트랜잭션이 롤백된 뒤에 실행되므로
