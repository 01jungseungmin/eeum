 ...
 * <p>
 * 웹 브라우저(사장님/관리자 대시보드)에서 알림 배지 카운트를 실시간으로 수신할 수 있게
 * 롱 폴링 없이 서버→클라이언트 단방향 스트림을 유지한다.
 * <p>
 * 연결은 계정 단위로 1개만 유지한다. 동일 계정이 재연결하면 기존 emitter를 대체한다.
 */
@Slf4j
@Component
public class SseEmitterManager {
    ...
    /**
     * 클라이언트가 SSE 구독을 시작할 때 호출.
     * 기존 연결이 있으면 완료(close) 처리 후 새 emitter로 교체한다.
     *
     * @param accountId 구독자 account ID
    ...
    /**
     * 안 읽은 알림 수를 해당 계정의 SSE 채널로 전송.
     * 연결된 emitter가 없으면 조용히 스킵한다.
     *
     * @param accountId 수신 계정 ID
     * @param count     안 읽은 알림 수
     */
