...
@RestController @RequiredArgsConstructor @RequestMapping("/admin/settlements")
public class AdminSettlementController {
 ...
 @PostMapping("/{id}/claim") public ResponseEntity<ApiResponse<String>> claim(@PathVariable Long id){String token=UUID.randomUUID().toString(); payoutService.claim(SecurityUtil.getCurrentAccountId(),id,token,LocalDateTime.now().plusMinutes(10));return ResponseEntity.ok(ApiResponse.success(token));}
 @PostMapping("/{id}/complete") public ResponseEntity<ApiResponse<Void>> complete(@PathVariable Long id,@Valid @RequestBody ManualPayoutCompleteRequestDto r){payoutService.complete(SecurityUtil.getCurrentAccountId(),id,r.claimToken(),r.payoutReference());return ResponseEntity.ok(ApiResponse.success());}
}
