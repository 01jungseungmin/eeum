...
@RequestMapping("/owner/settlements")
public class OwnerSettlementController {
    ...
    @GetMapping("/revenues")
    public ResponseEntity<ApiResponse<Page<OwnerRevenueResponseDto>>> revenues(
            @PageableDefault(size = 20) Pageable pageable
    ) {
        return ResponseEntity.ok(ApiResponse.success(
                settlementQueryService.getOwnerRevenues(SecurityUtil.getCurrentAccountId(), pageable)));
    }

    @Operation(summary = "사장 주간 정산 조회")
    @GetMapping("/weekly")
    public ResponseEntity<ApiResponse<Page<WeeklySettlementResponseDto>>> weekly(
            @PageableDefault(size = 20) Pageable pageable
    ) {
        return ResponseEntity.ok(ApiResponse.success(
                settlementQueryService.getOwnerWeeklySettlements(SecurityUtil.getCurrentAccountId(), pageable)));
    }
}
