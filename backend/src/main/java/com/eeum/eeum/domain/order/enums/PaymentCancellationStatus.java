 ...
 * 전액 취소 작업의 진행 단계.
 *
 * <p>외부 PG 취소는 롤백되지 않는다. 그래서 "PG를 취소했는가"와 "내부에 반영했는가"를
 * 한 트랜잭션의 성패로 뭉뚱그리지 않고 단계로 나눠 기록한다. 내부 반영이 실패하면
 * 롤백이 아니라 {@link #MANUAL_REVIEW_REQUIRED}로 남겨 사람이 수습할 수 있게 한다.
 */
public enum PaymentCancellationStatus {
     ...
     * 사람이 확인해야 하는 상태.
     *
     * <p>PG는 취소됐는데 내부 반영이 실패했거나, PortOne이 REQUESTED로 응답해
