...
public class ChatMessageRepositoryImpl implements ChatMessageRepositoryCustom {
    ...
    /**
     * 커서(keyset) 페이징. 페이지 번호가 없는 계약이라 CursorSlice로 돌려준다 —
     * Slice로 돌리면 두 번째 페이지에도 number=0, first=true가 실려 위치를 잘못 설명한다.
     *
     * 다음 커서는 서버가 만들어 응답에 싣는다. 클라이언트는 그대로 되돌려보내면 된다.
     */
    @Override
     ...
     *
     * 두 번째 항이 빠지면 같은 시각에 저장된 메시지가 페이지 경계에서 통째로 사라진다 —
     * sentAt < cursor 하나로는 그 시각의 나머지 메시지를 다시 볼 방법이 없다.
     */
