...
public class UsedProductRepositoryImpl implements UsedProductRepositoryCustom {
     ...
     *
     * 기본 정렬이 최신순이라 새 글이 맨 앞에 꽂힌다. OFFSET은 페이지 사이 등록 한 건에
     * 목록 전체가 밀려 경계 항목이 다음 페이지에서 중복으로 나온다(삭제되면 반대로 건너뛴다).
     *
     * 다음 커서는 서버가 만들어 응답에 싣는다(nextCursorValue). 정렬 키가 무엇이냐에 따라
     * 값의 형식이 달라지므로, 그 규칙을 아는 쪽이 만들어야 클라이언트가 정렬마다 다른 조립을
     * 하지 않는다. 페이지 번호가 없는 계약이라 Slice가 아니라 CursorSlice로 돌려준다 —
     ...
     *
     * 가격제안 글(price null)에서는 null을 돌려준다 — 그 구간은 정렬상 맨 뒤라
     * 게시글 ID만으로 이어 읽는다. afterCursor의 NULL 분기와 짝을 이룬다.
     */
     ...
     *
     * tie 항이 빠지면 정렬 키가 같은 글들 사이에서 경계를 끊지 못해 OFFSET과 같은
     * 중복·누락이 그대로 재현된다 — 같은 초에 올라온 글, 같은 가격의 글에서 실제로 난다.
     *
     * 가격순에서 가격제안(price null) 글은 정렬상 맨 뒤다. 그래서 값이 있는 커서에는
     * price IS NULL 구간을 함께 붙이고, 커서 자체가 그 구간이면 ID로만 이어 읽는다.
    ...
    /**
     * 적용할 정렬 확정 — 허용 필드 하나와 tie-break.
     *
     * 정렬 키를 하나로 좁히는 것은 커서 페이징의 전제다. 키가 둘 이상이면 커서가 그 값을
     * 모두 담아야 하고, 클라이언트는 정렬 조합마다 다른 커서를 만들어야 한다.
     * 실제로 이 목록은 한 번에 한 기준으로만 정렬하므로 첫 번째 허용 필드만 쓴다.
     * 무엇이 적용됐는지는 응답 CursorSlice.sort에 그대로 실려 나간다.
     */
     ...
     *
     * 요청 파라미터(sort=price,asc)로는 NULL 처리를 지정할 수 없어 항상 NATIVE로 들어온다.
     * 그대로 두면 응답 메타데이터는 NATIVE(MySQL ASC 기준 NULL이 앞)라고 말하는데 실제 결과는
     * NULL이 뒤에 오는, 정반대 상태가 된다. 여기서 확정하고 SQL은 이 값을 따라간다.
     */
