...
public class Account extends BaseEntity {
     ...
     *
     * 개인정보가 이미 파기된 계정은 되돌리지 않는다. 되살리면 email·이름·전화가 지워진 채
     ...
     *
     * 계정 행을 물리 삭제하려면 이 계정을 참조하는 18개 테이블(주문·결제·신고·채팅 등)을
     * 함께 정리해야 하는데, 주문·결제는 정산과 보존 의무가 걸려 지울 수 없다.
     * 파기해야 하는 것은 식별 정보이지 활동 이력이 아니므로, 행을 남기고 값만 지운다.
     *
     * email·nickname에는 UNIQUE 제약이 있다. 고정값으로 지우면 두 번째 탈퇴자부터 충돌하고
     ...
     * 이 토큰이 현재 세대인지.
     *
     * ver claim이 없는 토큰(이 기능 도입 전에 발급된 것)은 무효로 본다.
     ...
     * 화면 표시용 이름.
     *
     * name은 실명이다. 상대에게 보이는 자리에는 반드시 이 값을 쓴다.
     * 중고거래 1:1 문의처럼 모르는 사람과 연결되는 경로에서 실명이 그대로 노출된다.
     *
     * nickname은 UNIQUE이고 익명화(#anonymize()) 시에도 채워지므로
