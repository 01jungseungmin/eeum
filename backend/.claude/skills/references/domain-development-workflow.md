...
- 완료 기준은 "해당 Service가 동작한다"가 아니라 **"모든 진입점·경쟁 경로·외부 계약에서
  같은 불변식이 유지된다"**이다. 정상 경로가 통과했다는 사실만으로 단계를 닫지 않는다.
...
결정표를 채우기 전에 이 대상을 건드리는 **모든 쓰기 경로**를 먼저 나열한다.

- 사용자 API
- 관리자 API
- 신고 조치 (`ReportTargetActionExecutor` 구현체)
- 탈퇴·정리 스케줄러 (`AccountWithdrawalProcessor`, `*CleanupScheduler`)
- 내부 cascade 메서드 (`deleteAllByRefTypeAndRefId` 같은 도메인 간 정리 호출)

잠금 대상과 순서는 이 목록 위에서 결정한다. 목록 없이 사용자 API만 보고 잠금을 정하면,
계정 잠금을 잡지 않는 경로(관리자·신고 조치)가 나중에 드러나 잠금 규약이 어긋난다.
그 상태로 만든 동시성 테스트는 같은 사용자끼리의 경쟁만 관측해 정작 필요한 잠금을
검증하지 못한다.

각 진입점에 대해 actor(사용자/관리자/시스템), 잠그는 행, 잠금 순서, 대상 상태 요구조건을
한 줄씩 적는다.
...
- 쓰기 진입점 목록이 완성됐고, 각 진입점의 잠금 대상·순서가 하나의 전역 순서로 통일됐다.
...
- 매퍼·팩토리(`*ResponseDto.from()`)가 실제로 채우는 값과 `@Schema` 설명·예시의 일치.
  DTO 정의만 읽으면 "설명은 있는데 매퍼가 채우지 않는 필드", "설명과 다른 값이 담기는 필드"를
  놓친다. 필드 목록이 아니라 매퍼 본문과 스키마를 나란히 놓고 대조한다.
...
### 회귀 검증 — 그 테스트가 실제로 무엇을 잡는지 증명한다

동시성·회귀 테스트는 **통과하는 것만으로는 아무것도 보증하지 않는다.** 지키려는 프로덕션
코드(잠금, current read, 조건부 UPDATE)를 일부러 되돌린 뒤 그 테스트가 실제로 실패하는지
확인하고, 확인 후 원래대로 복구한다.

되돌렸는데도 통과한다면 그 테스트는 다른 무언가에 의해 우연히 지켜지고 있는 것이다.
실제 사례:

- 잠금 전에 일반 SELECT가 없는 경로로 테스트를 짜면, MySQL REPEATABLE READ의 스냅샷이
  잠금 획득 이후에 잡혀 stale read 경쟁이 재현되지 않는다.
- 두 요청의 actor가 같으면 계정 잠금에서 먼저 직렬화돼, 정작 검증하려던 대상 행 잠금은
  제거해도 테스트가 통과한다.

실패 메시지가 "무엇이 깨졌는지"를 말하는지도 함께 본다. 잠금이 사라졌을 때 불변식 위반보다
"경쟁이 재현되지 않음"이 먼저 걸리는 것은 정상이며, 그 자체가 직렬화 상실의 신호다.

### Gate 6

- 새로 만든 회귀·동시성 테스트는 되돌림 검증으로 검출력을 확인했고, 프로덕션 코드를
  원래대로 복구했다(`git diff -- src/main`이 의도한 변경만 남아 있다).
