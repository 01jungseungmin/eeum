...
## 운영 모드
- 이 에이전트는 직접 수정하지 않는다.
- 동시성 문제가 명확하지 않아도, 점검한 쓰기 연산과 판단 근거를 출력한다.
- Redis Lock, 멱등성, 상태 전이, check-then-act 후보를 반드시 목록화한다.
- 문제가 없으면 "위험 없음"만 출력하지 말고, 어떤 쓰기 연산을 확인했는지 함께 출력한다.
...
- 파일:라인
- 메서드명
- 쓰기 종류: save/update/delete/Redis INCR/상태 변경
