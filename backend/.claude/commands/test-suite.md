---
description: 아키텍처·단위·통합·자원 테스트를 계층 순서로 실행하고 실패를 원인별로 분류해 보고
allowed-tools: Read, Grep, Glob, Bash
---

전체 테스트 스위트를 실행하고 결과를 진단하세요.
$ARGUMENTS 로 계층 이름(`arch`, `unit`, `integration`, `resource`)이나 도메인명이 주어지면 그 범위만 실행합니다.

## 목적

`./gradlew test`는 "몇 개 실패"만 알려준다. 이 커맨드는 **실패가 어느 계층에서 왜 났는지**까지 판정한다.

특히 자원 계열 실패(스레드 블로킹, 커넥션 고갈)는 증상이 전부 "타임아웃"으로 똑같이 보여서
로그만으로는 원인을 구분할 수 없다. 이 커맨드는 `references/resource-budget.md`의 장애 모드 카탈로그에 매핑해 판정한다.
...
| 2 | 단위 | `./gradlew test --tests "*Test" -PexcludeIntegration` 가 없으면 전체 실행 후 결과에서 분리 | 로직 분기 |
...
- **테스트나 프로덕션 코드를 수정하지 않는다.** 이 커맨드는 실행과 진단만 한다. 수정은 `build-fixer` 또는 명시적 요청으로 진행한다.
...
3. **우선순위 제안** — 자원 장애 > 아키텍처 위반 > 로직 회귀 > 테스트 결함 순. 각각 어느 커맨드로 이어가면 되는지 명시 (`regression-test`, `arch-rules`, `build-fixer`)
