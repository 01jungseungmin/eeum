...
import {
  WEEKLY_SETTLEMENT_STATUS_LABEL,
  WEEKLY_SETTLEMENT_CLAIMABLE_STATUSES,
...
function SettlementTable({ settlements, loading, onProcessClick }) {
  return (
    <Card>
      {loading ? (
        <EmptyText>불러오는 중...</EmptyText>
      ) : settlements.length === 0 ? (
        <EmptyText>조회된 정산 내역이 없어요.</EmptyText>
      ) : (
        <TableWrapper>
          <Table>
            <thead>
            ...
            <tbody>
              {settlements.map((s) => (
                <tr key={s.weeklySettlementId}>
                  ...
                  <td>
                    {WEEKLY_SETTLEMENT_CLAIMABLE_STATUSES.includes(
                      s.status,
                    ) && (
                      <ActionButton onClick={() => onProcessClick(s)}>
                        지급 처리
                      </ActionButton>
                    )}
                  </td>
